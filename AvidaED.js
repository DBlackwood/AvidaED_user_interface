...
//  "dojox/charting/Theme",
  ...
  function doPopMap() {
    ...
    switch(dijit.byId("colorMode").value) {
      case 'Fitness':
        colorMsg.fill = msg.fitness.data;
        str = 'last_fitness';
  ...
  function findLogicOutline(msg) {
    for (ii=0; ii < msg.not.length; ii++) {colorMsg.out[ii] = 0}
    if ('on' == document.getElementById(notButton).value){ for (ii=0; ii < msg.not.length; ii++) {colorMsg.out[ii] = colorMsg.out[ii] * msg.not[ii];} }
  }
...
//Draw Cell outline or including special case for Selected
  ...
  function DrawChildren() {  //Draw the children of parents
    var cc, rr, xx, yy;
    for (ii=0; ii< colorMsg.fill.length; ii++) {
      cc = ii % grd.cols;
      rr = Math.trunc(ii/grd.cols);
      xx = grd.marginX + grd.xOffset + cc*grd.cellWd;
      yy = grd.marginY + grd.yOffset + rr*grd.cellHt;
      if (nan == colorMsg.fill[ii]) {cntx.fillStyle='#000'}
      else {cntx.fillStyle = parents.color[colorMsg.fill[ii]]}
      cntx.fillRect(xx, yy, grd.cellWd-1, grd.cellHt-1);
    }
  }
  ...
  function GradientScale() {
    ...
    if (true) {  colorMsg.max = 805040;
      ...
      var marks = 4;
      var txt = "";
      if (colorMsg.max > 4000) {place = 0}
      else if (colorMsg.max > 400) {place = 1}
      for (var ii= 0; ii<=marks; ii++) {
        xx = ii*colorMsg.max/marks;
